...
// This function calculates the hadronic chi2 - SNT version
bool getMTopChi2(vector<LorentzVector> jets, vector<float> sigma_jets, vector<bool> btag,LorentzVector lep1){
        int selj1_idx = -9999;
        int selj2_idx = -9999;
        int selj3_idx = -9999;
        ...
        for ( int b=0; b<n_jets; ++b ) {
          if(b==selj1_idx || b==selj2_idx || b==selj3_idx){
            if(ROOT::Math::VectorUtil::DeltaR(lep1,jets[b]) > 3.0) drflag = false; 
          }
        }
        if(mtopchi2>270.) mtopflag = false;

        return drflag*mtopchi2;
