...
void SetTDRStyle(){
    ...
    TStyle *tdrStyleAG = new TStyle("tdrStyleAGAG","Style for P-TDR");
...
void dataMCplotMaker(TH1F* Data_in, std::vector <std::pair <TH1F*, TH1F*> > Backgrounds_pair_in, std::vector <std::string> Titles, std::string titleIn, std::string title2In, std::string options_string, std::vector <TH1F*> Signals_in, std::vector <std::string> SignalTitles, std::vector <Color_t> color_input){
  ...
  if (!noData && !noRatioPlot) stack->GetYaxis()->SetTitleOffset(1.2+yTitleOffset_);
...
/*
  //Stack of backgrounds
  THStack *stack_err = new THStack("stack_err", "");
//  Data->SetMarkerStyle(20);
//  Data->UseCurrentStyle();
  if(!compareMultiple && noFill == 0 && Signals.size() >= 5 && !nostack){
    std::vector <int> Style;
    ...
    for (unsigned int i = 0; i < Backgrounds.size(); i++){
      ...
      Backgrounds[i]->SetLineColor(kGray);
      Backgrounds[i]->SetFillStyle(Style[i]);
if(i==3)   Backgrounds[3]->SetFillColor(kBlue);
if(i==3)   Backgrounds[3]->SetFillStyle(3018);
      stack_err->Add(Backgrounds[i]);
    }
  }
  if(noFill == 0 && (Signals.size() < 5 || nostack || compareMultiple)){
    for (unsigned int i = 0; i < Backgrounds.size(); i++){
...
*/
  //Draw
  if (!nostack && !dots && histoErrors){
  stack->Draw("histe");

  //stack_err->Draw("e2same");
  }else if (!nostack && !dots && !histoErrors) stack->Draw("hist");
  else if (dots) stack->Draw("Enostack"); 
  ...
  if(noErrBars) stack2->Draw("SAME");
  else stack2->Draw("SAMEE");
...
//background_syst->SetFillStyle(3018);
//background_syst->SetFillColor(kBlue);

  if (Background_systs.size() > 0){
    background_syst = new TH1F(*Background_systs[0]); 
    if (!background_syst->GetSumw2N()) background_syst->Sumw2(); 
    background_syst->SetFillColor(kBlue+1); //should have an option for these?
    background_syst->SetFillStyle(3018); 
  }
...
//  if (background_syst != 0 && do_background_syst == true) 
//background_syst->SetFillStyle(3018);
//background_syst->SetFillColor(kBlue);
...
//  else stack2->Draw("PSAMEE");
  ...
  else leg = new TLegend(0.46+legendRight,0.75+legendUp,0.78+legendRight+legendWider_,0.9+legendUp+legendTaller_);
  ...
  if ((!noData || compareMultiple) && !noRatioPlot){
    ...
    err_hist->GetYaxis()->SetTitleOffset(1.2);
    ...
    else if(noErrBars) err_hist->Draw("");
    else err_hist->Draw("E");
    ...
    else if(noErrBars) err_hist->Draw("SAME");
    else err_hist->Draw("ESAME");
    TH1F* err_hist_sys2 = (TH1F*)err_hist->Clone("err_hist_sys2");
    err_hist_sys2 ->Reset();
/*   err_hist_sys2->SetBinContent(1,err_hist->GetBinContent(1));
   err_hist_sys2->SetBinContent(2,err_hist->GetBinContent(2));
    err_hist_sys2->SetBinError(1,err_hist->GetBinContent(1)*0.02);
cout<<"error"<<err_hist->GetBinContent(2)*0.15<<endl;
    err_hist_sys2->SetBinError(2,0.139338);
cout<<"error"<<err_hist_sys2->GetBinError(2)<<endl;
//background_syst->SetFillStyle(3018);
////background_syst->SetFillColor(kBlue);

    err_hist_sys2->SetFillStyle(3018);
    err_hist_sys2->SetFillColor(kBlue);
    err_hist_sys2->Draw("sameE2");
*/
    err_hist->GetXaxis()->SetLabelSize(0);
    err_hist->GetYaxis()->SetLabelSize(0.2);
    err_hist->GetYaxis()->SetRangeUser(0., 2.0);
