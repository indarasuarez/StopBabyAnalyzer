...
void StopCMS3::Init(TTree *tree) {
  ...
  tree->SetMakeClass(1);
	...
	MT2_l_l_branch = 0;
	if (tree->GetBranch("MT2_l_l") != 0) {
		MT2_l_l_branch = tree->GetBranch("MT2_l_l");
		if (MT2_l_l_branch) {MT2_l_l_branch->SetAddress(&MT2_l_l_);}
	}
	MT2_lb_b_branch = 0;
	...
	// load all branches
{
	...
	const float &StopCMS3::weight_ISRnjets()
	{
	...
	const float &StopCMS3::weight_ISRnjets_UP()
	{
	...
	const float &StopCMS3::weight_ISRnjets_DN()
	{
	...
	const int &StopCMS3::NnonISRjets()
	{
	...
	const bool &StopCMS3::filt_fastsimjets()
	{
	...
	const float &StopCMS3::filt_badMuonFilter()
	{
	...
	const float &StopCMS3::dR_lep_leadb()
	{
	...
	const float &StopCMS3::dphi_Wlep()
	{
	...
	const float &StopCMS3::MET_over_sqrtHT()
	{
	...
	const float &StopCMS3::MT2_l_l()
	{
		if (not MT2_l_l_isLoaded) {
			if (MT2_l_l_branch != 0) {
				MT2_l_l_branch->GetEntry(index);
			} else { 
				printf("branch MT2_l_l_branch does not exist!\n");
				exit(1);
			}
			MT2_l_l_isLoaded = true;
		}
		return MT2_l_l_;
	}
	const float &StopCMS3::MT2_lb_b()
	{
	...
	const float &StopCMS3::MT2_lb_b_mass()
	{
	...
	const float &StopCMS3::MT2_lb_bqq()
	{
	...
	const float &StopCMS3::MT2_lb_bqq_mass()
	{
	...
	const float &StopCMS3::Mlb_lead_bdiscr()
	{
	...
	const float &StopCMS3::Mjjj_lep2()
	{
...
namespace stoptas {
	...
	const float &weight_ISRnjets() { return stopcms3.weight_ISRnjets(); }
	const float &weight_ISRnjets_UP() { return stopcms3.weight_ISRnjets_UP(); }
	const float &weight_ISRnjets_DN() { return stopcms3.weight_ISRnjets_DN(); }
	const int &NISRjets() { return stopcms3.NISRjets(); }
	const int &NnonISRjets() { return stopcms3.NnonISRjets(); }
	const bool &filt_fastsimjets() { return stopcms3.filt_fastsimjets(); }
	...
	const float &nupt() { return stopcms3.nupt(); }
	...
	const float &filt_badMuonFilter() { return stopcms3.filt_badMuonFilter(); }
	...
	const float &MT2_l_l() { return stopcms3.MT2_l_l(); }
